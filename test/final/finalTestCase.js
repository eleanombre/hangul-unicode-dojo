...
const eleanorsSolution = require('../helpers/index');
...
function createRandomTestCase() {
  let testArr = [];
  testArr.push(ranIndex(initial), ranIndex(medial), ranIndex(final));
  return testArr;
}
...
describe('Final Test Suite', () => {
	...
	for ( let i = 0 ; i < 100; i ++ ) {
		...
		let expected = eleanorsSolution(arr);
		let result = solution(arr);

		it(`should respond with ${expected} when given ${arr}`, () => {
